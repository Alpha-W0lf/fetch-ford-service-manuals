...
**Workshop auth-budget stop (Guide 04.4):** Inside `saveEntireManual`, after `WORKSHOP_AUTH_STOP_THRESHOLD` consecutive auth-class failures (`auth` or `subscription-expired`), the worker logs `[auth-budget-stop]`, records gaps, and returns normally (exit 0 with blocking gaps → orchestrator `incomplete`). Cookie refresh via `WORKSHOP_AUTH_REFRESH_THRESHOLD` runs **at most once per worker run** so the stop threshold can accumulate after a refresh attempt. TreeAndCover and wiring-TOC auth failures record early gaps via `src/jobHelpers.ts` without throwing. When the orchestrator sees `[auth-budget-stop]` in the vehicle log, `recordOutcome` counts toward cooldown even if runtime exceeded `VEHICLE_FAST_FAIL_SEC` (slow partial runs).
