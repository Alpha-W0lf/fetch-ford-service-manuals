  ...
  if (final.status === "incomplete") {
    ...
    if (authClass) {
      ...
      if (orchestratorState?.vehicleCooldown) {
        ...
        let authBudgetStop = false;
        try {
          authBudgetStop = fs
            .readFileSync(logPath, "utf8")
            .includes("[auth-budget-stop]");
        } catch {
          /* */
        }
        ...
        if (cd.excluded) {
          const kind = authBudgetStop ? "auth-budget" : "fast auth";
          deps.log(
            `[cooldown] ${vid} excluded for ${config.vehicleCooldownSec}s after ${cd.consecutive} ${kind} incomplete(s)`
          );
        }
      }
    }
